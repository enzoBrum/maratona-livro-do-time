struct LCA {
  ...
  LCA(vector<vector<int>> &_tree, map<ii, int> &_weights, int root)
      : tree(_tree), weights(_weights) {
    n = tree.size();
    tin.resize(n);
    tout.resize(n);
    timer = 0;
  ...
  void dfs(int v, int p) {
    ...
    for (int i = 1; i <= l; ++i) {
      up[v][i] =
          up[up[v][i - 1]][i - 1]; // 2^i up from v is the same as 2^(i-1) up
                                   // from (2^(i-1) up from v [its parent]))
      upMaxEdge[v][i] =
          max(upMaxEdge[v][i - 1],
              upMaxEdge[up[v][i - 1]]
                       [i - 1]); // max from first half and second half
    }
